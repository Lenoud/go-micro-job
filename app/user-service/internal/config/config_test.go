...
func TestLoadUserServiceConfigs(t *testing.T) {
	tests := []struct {
		name     string
		path     string
		wantMode string
	}{
		{name: "local", path: "../../etc/user-local.yaml", wantMode: "dev"},
		{name: "prod", path: "../../etc/user.yaml", wantMode: "pro"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if cfg.DevServer.Port != 6061 {
				t.Fatalf("expected dev server port 6061, got %d", cfg.DevServer.Port)
			}
