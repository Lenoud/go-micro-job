...
// RpcBase extracts code/msg from a gRPC response into a BaseResp.
// All user-service proto responses implement GetCode()/GetMsg().
func RpcBase(resp interface {
	GetCode() int64
	GetMsg() string
}) types.BaseResp {
	return types.BaseResp{Code: resp.GetCode(), Msg: resp.GetMsg(), Timestamp: currentTimeMillis()}
}
