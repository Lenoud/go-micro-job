...
招聘系统的微服务版本，从父仓库 `go-server-resume` 拆分而来。多个 go module 通过 `go.work` 管理：
...
- **department-service**（`app/department-service/`）: gRPC 服务，端口 9102，部门管理
- **oplog-service**（`app/oplog-service/`）: gRPC 服务，端口 9103，操作日志读写
...
cd app/department-service && go build ./...
...
```
micro/
├── go.work                    # Go workspace: api-gateway + user-service + department-service + oplog-service
├── docker-compose.yml         # 基础设施 + 生产部署
├── scripts/                   # dev.sh, curl-test.sh, stop-dev.sh
├── sql/                       # 微服务专用 SQL（oplog.sql 等）
├── app/
...
│   │   └── internal/
│   │       ├── config/        # Config 结构体
│   │       ├── handler/       # HTTP handler（按模块分目录：user/、oplog/）
│   │       ├── logic/         # 业务逻辑（按模块分目录，调用 gRPC client）
│   │       ├── common/        # accesslog、oplog_converter、oplog_identity
│   │       ├── svc/           # ServiceContext，注入各 RPC client
│   │       └── types/         # 请求/响应类型（goctl 生成）
│   ├── user-service/          # gRPC 用户服务 (go-zero zrpc)
...
│       ├── etc/
│       └── internal/
└── shared/                    # 微服务间共享（如 proto 类型）
```

**调用链**: HTTP → api-gateway handler → logic → gRPC client → 各 gRPC service server → logic → model → MySQL

**api-gateway 是薄层**: handler 仅转发请求，logic 做 JSON 序列化/反序列化（gRPC `ApiResponse.data` 是 JSON string），业务逻辑在各 gRPC service 中。api-gateway 还负责操作日志的异步写入（通过 `common/accesslog.go` 的 `OpLogWriter`）。

## 代码生成

### api-gateway（goctl api）

`desc/main.api` 是 API 定义入口（import `desc/*.api`）。修改 API 定义后：
...
5. 修改 `desc/main.api`（及对应 `desc/*.api`）添加 HTTP 端点
...
### 统一响应
- 各 gRPC service logic 返回类型化 protobuf 响应，用各模块的 `common.OkXxx(msg, data)` / `common.FailXxx(msg)` 等 helper 构造
- api-gateway logic 从 gRPC 响应提取数据，构造 HTTP 类型化响应（嵌入 `BaseResp`）
- HTTP 响应格式：`{"code": 200, "msg": "success", "data": ..., "timestamp": ...}`

### Timestamp 设计
- **各 gRPC service**：业务逻辑执行处，每个响应 helper 内部调用 `currentTimeMillis()` 生成 timestamp
- **api-gateway**：两条路径互不干扰：
  - gRPC 成功响应 → `RpcBase()` 透传 gRPC service 的 timestamp（反映业务处理时间）
...
参照 user-service / department-service / oplog-service 的模式：
1. 创建新的 protobuf 定义文件
2. 用 goctl rpc 生成 gRPC 服务骨架
3. 实现业务逻辑（logic → model）
4. 在 api-gateway 的 `desc/main.api`（及对应 `desc/*.api`）中添加 HTTP 端点
...
8. 更新 `dev.sh` 启动脚本
9. 父仓库 `scripts/dev.sh --micro` 中同步添加新服务的启动/停止逻辑
